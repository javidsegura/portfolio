...
export default function TechStackCard({ TECHNOLOGIES_ELEMENTS, title, className, Icon }: TechStackCardProps){
      return (
            <div className={`w-full md:w-fit p-5 md:p-10 bg-card border border-border rounded-3xl shadow-lg hover:shadow-xl transition-all duration-300 ease-out h-auto ${className ?? ""}`}> 
                  ...
                  <div id="technology-items" className="flex flex-wrap gap-2">
                        {TECHNOLOGIES_ELEMENTS.map((element, index) => {
                              return (
                                    <div id="framework" key={index} className="bg-secondary flex flex-row flex-wrap gap-1 rounded-3xl md:min-w-[calc(33.333%-8px)] px-2 py-1 my-0.5">
